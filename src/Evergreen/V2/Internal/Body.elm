module Evergreen.V2.Internal.Body exposing (..)

import Evergreen.V2.Internal.Coordinates
import Evergreen.V2.Internal.Material
import Evergreen.V2.Internal.Matrix3
import Evergreen.V2.Internal.Shape
import Evergreen.V2.Internal.Transform3d
import Evergreen.V2.Internal.Vector3


type alias Geometry =
    { volume : Float
    , shapesWithMaterials : List ( Evergreen.V2.Internal.Shape.Shape Evergreen.V2.Internal.Shape.CenterOfMassCoordinates, Evergreen.V2.Internal.Material.Material )
    , boundingSphereRadius : Float
    }


type alias Body =
    { id : Int
    , kindInt : Int
    , transform3d :
        Evergreen.V2.Internal.Transform3d.Transform3d
            Evergreen.V2.Internal.Coordinates.WorldCoordinates
            { defines : Evergreen.V2.Internal.Shape.CenterOfMassCoordinates
            }
    , centerOfMassTransform3d :
        Evergreen.V2.Internal.Transform3d.Transform3d
            Evergreen.V2.Internal.Coordinates.BodyCoordinates
            { defines : Evergreen.V2.Internal.Shape.CenterOfMassCoordinates
            }
    , velocity : Evergreen.V2.Internal.Vector3.Vec3
    , angularVelocity : Evergreen.V2.Internal.Vector3.Vec3
    , mass : Float
    , geometry : Geometry
    , worldShapesWithMaterials : List ( Evergreen.V2.Internal.Shape.Shape Evergreen.V2.Internal.Coordinates.WorldCoordinates, Evergreen.V2.Internal.Material.Material )
    , force : Evergreen.V2.Internal.Vector3.Vec3
    , torque : Evergreen.V2.Internal.Vector3.Vec3
    , linearDamping : Float
    , angularDamping : Float
    , invMass : Float
    , invInertia : Evergreen.V2.Internal.Vector3.Vec3
    , invInertiaWorld : Evergreen.V2.Internal.Matrix3.Mat3
    , linearLock : Evergreen.V2.Internal.Vector3.Vec3
    , angularLock : Evergreen.V2.Internal.Vector3.Vec3
    }
